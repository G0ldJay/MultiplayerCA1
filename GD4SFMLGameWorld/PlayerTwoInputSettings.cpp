...
bool PlayerTwoInputSettingsState::handleEvent(const sf::Event& event)
{
	...
	for (std::size_t action = 0; action < static_cast<int>(ActionID::ActionCount); ++action)
	{
		...
		else if (mBindingButtonsController[action]->isActive())
		{
			...
			{
				getContext().playerTwo->assignJoystickButton(static_cast<ActionID>(action), event.joystickButton.button);
...
void PlayerTwoInputSettingsState::updateControllerLabels()
{
	...
	for (std::size_t i = 0; i < static_cast<int>(ActionID::ActionCount); ++i)
	{
		int buttonNumber = playerTwo.getAssignedJoypadButton(static_cast<ActionID>(i));
